...
<template>
  ...
  >
    <div
      class="flex flex-col sm:flex-row gap-2 justify-between items-center sm:gap-1 mb-2 p-5 rounded-t top-summary"
    >
      <div class="flex flex-col gap-1">
        <h2 class="text-lg font-bold">{{ scannedUrl }}</h2>
      </div>
      <div class="flex flex-col gap-1">
        <p class="malicious rounded-md p-2" v-if="maliciousResult">
          There is a high probability that this URL is malicious
        </p>
        <p
          class="suspicious rounded-md p-2"
          v-if="suspiciousResult && !maliciousResult"
        >
          There is a medium probability that this URL is malicious
        </p>
        <p
          class="harmless rounded-md p-2"
          v-if="!maliciousResult && !suspiciousResult"
        >
    ...
    <div class="status-container flex flex-wrap p-5 gap-2">
      ...
      <p class="suspicious rounded-md p-2">
        Suspicious: {{ status.suspicious }}
      </p>
      <p class="undetected rounded-md p-2">
